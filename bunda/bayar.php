...
$tt = mysqli_fetch_assoc(mysqli_query($conn, "SELECT SUM(IF(via = 'Transfer', nominal, 0)) as tf, SUM(IF(via = 'Cash', nominal, 0)) as cs FROM bp_daftar a JOIN tb_santri b ON a.nis = b.nis WHERE b.ket = 'baru'"));
?>

<!-- Container Fluid-->
<div class="container-fluid" id="container-wrapper">
    <div class="d-sm-flex align-items-center justify-content-between mb-4">
        <h1 class="h3 mb-0 text-gray-800">Biaya Pendaftaran Santri Baru</h1>
        <ol class="breadcrumb">
                ...
                <div class="table-responsive p-3">
                    <table class="table align-items-center table-flush table-hover table-sm table-bordered" id="dataTableHover" style="font-size: 13px;">
                        ...
                        <tbody>
                            <?php
                            $no = 1;
                            $sql = mysqli_query($conn, "SELECT a.*, b.* FROM bp_daftar a JOIN tb_santri b ON a.nis=b.nis WHERE b.ket = 'baru' ORDER BY a.created ASC ");
                            ...
                            ?>
                                <tr>
                                    ...
                                    <td>
                                        <a onclick="return confirm('Yajin akan dihapus ?')" href="<?= 'hapus.php?kd=hpend&id=' . $row['id_bayar'] ?>"><button class="btn btn-sm btn-danger"> Del</button></a>
                                        <a onclick="return confirm('Yajin akan dikirim ?')" href="<?= 'kirim.php?kd=df&id=' . $row['id_bayar'] ?>"><button class="btn btn-sm btn-primary"><i class="fa fa-paper-plane"></i></button></a>
                                    </td>
                                </tr>
                            <?php } ?>
                        </tbody>
